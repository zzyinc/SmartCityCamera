...
cv::Mat similarTransform(const cv::Mat& src, const cv::Mat& dst) {
    ...
    if (rank == 0) {
        ...
        if (cv::determinant(U) * cv::determinant(V) > 0) {
            ...
            d.at<float>(dim - 1, 0) = static_cast<float>(s);
        }
    }
    else {
        cv::Mat diag_ = cv::Mat::diag(d);
    ...
    float val = static_cast<float>(cv::sum(var_).val[0]);
    cv::Mat res;
    cv::multiply(d, S, res);
    float scale = static_cast<float>(1.0f / val * cv::sum(res).val[0]);
